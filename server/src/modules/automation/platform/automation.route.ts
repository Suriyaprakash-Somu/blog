...
import { rssSources, automationTopicSessions, automationTopicCandidates, feedItems } from "../automation.schema.js";
...
const feedItemsCrudRoutes = createCrudRoutes({
    table: feedItems,
    cache: {
        tag: "feedItems",
        keyPrefix: "feedItem",
    },
    searchableColumns: ["title", "url", "guid"],
    columnMap: {
    ...
    validation: {
        // listQuery omitted (uses defaults)
    },
    access: CRUD_ACCESS.feedItems,
    routes: {
        list: true,
        detail: false,
        create: false,
...
export const automationRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    // Feed items (read-only list) for admin UI
    await fastify.register(feedItemsCrudRoutes, { prefix: "/feed-items" });
    ...
    fastify.post(
        ...
        async (request, reply) => {
            ...
            return { success: true, message: "Item ignored" };
        }
    );

    fastify.post(
        "/feed-items/:id/unignore",
        {
            preHandler: [
                requirePlatformAuth(),
                platformAbilityGuard(ACTIONS.UPDATE, SUBJECTS.PLATFORM_SETTINGS),
        ...
        async (request, reply) => {
            ...
            await db
                .update(feedItems)
                .set({ processingStatus: "unprocessed" })
                .where(eq(feedItems.id, id));
    ...
    fastify.post(
        ...
        async (request, reply) => {
            try {
                const { id } = request.params as { id: string };
                const result = await AutomationService.autoGenerateDraftFromFeedItemId(id);
                return {
                    success: true,
                    postId: result.id,
                    title: result.title,
