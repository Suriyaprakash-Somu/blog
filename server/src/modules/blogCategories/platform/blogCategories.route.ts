  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const messages = buildBlogCategoryPrompt(name, customPrompt);
        const result = await chatCompletionJSON<BlogCategoryGenerated>({
          messages,
          temperature: 0.7,
        });
        const validated = blogCategoryGeneratedSchema.parse(result);
