...
import type { RoleOption, TenantUser, TenantUserFormData } from "./types";
...
export function TenantUserForm({
  data,
  onSuccess,
}: OperationComponentProps<TenantUser>) {
  ...
  const mutation = useApiMutation<TenantUser, TenantUserFormData>({
    endpoint: isEdit
      ? tenantUsersApi.update.endpoint({ id: editData!.id })
      : tenantUsersApi.create.endpoint,
    method: isEdit ? "PUT" : "POST",
    revalidateTags: [tenantUsersApi.getList.key],
  });

  const handleSubmit = async (values: TenantUserFormData) => {
    try {
      const payload: TenantUserFormData = {
        ...values,
